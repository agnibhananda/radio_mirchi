...
export default function RegisterForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      (async () => {
        try {
          const createMissionResponse = await fetch(
            "http://localhost:8000/api/v1/create_mission",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (!createMissionResponse.ok) {
            console.error(
              "Failed to create mission in background:",
              missionData.message || "Unknown error"
            );
            // Optionally, you could set a non-blocking error state here if needed
          } else {
            console.log(
              "Mission created successfully. Mission ID from backend:",
              missionData._id
            );
          ...
          console.error(
            "Network error during mission creation in background:",
            error
          );
  ...
  const isFormValid =
    ...
    (mode === "signin" || formData.name.trim());
        ...
        <div className="p-4">
          ...
          <div className="text-center mb-3">
            ...
            <p className="text-gray-600 text-xs">
              {mode === "signup"
                ? "Register to track your infiltration progress"
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            ...
            <Button
              ...
              disabled={
                ...
                (mode === "signin" && !formData.password.trim())
              }
              className="w-full"
              color="primary"
            >
